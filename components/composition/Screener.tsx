...
export const Screener: React.FC = () => {
  ...
  const [selectedRegions, setRegions] = useState<number[]>([0]);
  ...
  return (
    <div className="p-16 flex flex-col gap-4">
      ...
      <FilterGroup items={filters.map((filter :Filter) => {return {display:filter.id, label:filter.label as string, onRemove:filter.onRemove as ()=>void}})} />
      ...
      hints={true} 
      rowsPerPage={25} 
      columnDetails={{public:["ticker", "name", currentPill?"assets":"market-cap"]}} 
      defSort="market-cap" />}
    </div>
  );
};
